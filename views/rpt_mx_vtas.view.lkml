view: rpt_mx_vtas {
  derived_table: {
    sql: SELECT *,DATE(PARSE_TIMESTAMP('%Y%m%d',CALDAY)) fecha,DATE_ADD(CURRENT_DATE(), INTERVAL -1 DAY) ACTUALIZACION FROM `eon-internal-bigquery.DATASET_RAW_ENVASES.rpt_vtas` where CATEGORY  not in ('TOTAL MXN') and CATEGORY not in ('OTROS','SUBPRODUCTOS') ;;
  }
